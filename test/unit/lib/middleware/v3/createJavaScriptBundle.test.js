...
describe('createJavaScriptBundle', function () {
	...
	context('when given a valid request', function () {
		...
		context('for a component with no default export', async function () {
			...
			it('adds an empty object to the global object with undefined default property', async () => {
				const context = {};
				context.self = context;
				script.runInNewContext(context);
				proclaim.isObject(context.Origami);
				proclaim.deepEqual(context.Origami['o-test-component'], {default: undefined});
			});
		});
