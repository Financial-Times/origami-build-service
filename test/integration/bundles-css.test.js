...
describe('GET /__origami/service/build/bundles/css', function() {
	this.timeout(20000);
	this.slow(5000);
	...
	describe('when a module is requested', function() {
		...
		before(async function () {
			response = await request(this.app)
				.get(`/__origami/service/build/bundles/css?modules=${moduleName}&newerthan=${now}`)
				.set('Connection', 'close');
		});
	...
	if (!process.env.HOST) {
		 ...
		 */
			...
			before(async function () {
				response = await request(this.app)
					.get(`/__origami/service/build/bundles/css?modules=${moduleName}`)
					.set('Connection', 'close');
			});
			...
			it('should ignore URL encoding when checking for static bundles', function() {
				const pathUnencoded = `/__origami/service/build/bundles/css?modules=${moduleName}^1.0.0`;
				const pathEncoded = `/__origami/service/build/bundles/css?modules=${moduleName}%5E1.0.0`;
				const expectedContent = '/* STATIC BUNDLE (bundles/css) */\n';
