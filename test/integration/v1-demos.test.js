...
describe('GET /__origami/service/build/v1/demos', function() {
	this.timeout(40400);
	this.slow(5000);
	...
	describe('when a valid module and demo are requested', function() {
		...
		before(async function () {
			response = await request(this.app)
				.get(`/__origami/service/build/v1/demos/${moduleName}/${pathName}`)
				.set('Connection', 'close');
		});
	...
	describe('when a valid module and no demo is requested', function() {
		...
		before(async function () {
			response = await request(this.app)
				.get(`/__origami/service/build/v1/demos/${moduleName}/`)
				.set('Connection', 'close');
		});
