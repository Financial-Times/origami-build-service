...
describe('GET /v2/bundles/css', function() {
	...
	describe('when a valid module is requested', function() {
		...
		it('should respond with the bundled CSS', function() {
			assert.deepEqual(response.text, '/** Shrinkwrap URL:\n *    /__origami/service/build/v2/bundles/css?modules=o-test-component%401.0.4%2Co-autoinit%401.5.1&shrinkwrap=&brand=master\n */\n#test-compile-error{color:red}');
		});
	...
	describe('when a module is requested for a valid source param', function () {
		...
		it('should respond with css for the given source', function () {
			assert.deepEqual(response.text, `/** Shrinkwrap URL:\n *    /__origami/service/build/v2/bundles/css?modules=o-test-component%401.0.34%2Co-autoinit%401.5.1&shrinkwrap=&brand=master\n */\n.o-test-component{content:"${source}"}`);
		});
	});
...
describe('GET /__origami/service/build/v2/bundles/css', function() {
	...
	describe('when a valid module is requested', function() {
		...
		before(async function () {
			const now = (new Date()).toISOString();
			response = await request(this.app)
				.get(`/__origami/service/build/v2/bundles/css?modules=${moduleName}&newerthan=${now}`)
				.redirects(5)
				.set('Connection', 'close');
		});
	...
	describe('when a valid module is requested (with no minification)', function() {
		...
		before(async function () {
			const now = (new Date()).toISOString();
			response = await request(this.app)
				.get(`/__origami/service/build/v2/bundles/css?modules=${moduleName}&newerthan=${now}&minify=none`)
				.redirects(5)
				.set('Connection', 'close');
		});
	...
	describe('when a valid module is requested with a shrinkwrap url', function () {
		...
		before(async function () {
			response = await request(this.app)
				.get(`/__origami/service/build/v2/bundles/css?modules=${modulesParam}&shrinkwrap=${shrinkwrapParam}&brand=master`)
				.redirects(5)
				.set('Connection', 'close');
		});
	...
	describe('when the shrinkwrap param includes a module which isn\'t on the allow list', function () {
		...
		before(async function () {
			const now = (new Date()).toISOString();
			response = await request(this.app)
				.get(`/__origami/service/build/v2/bundles/css?modules=${moduleName}&shrinkwrap=${shrinkwrapParameter}&newerthan=${now}`)
				.redirects(5)
				.set('Connection', 'close');
		});
	...
	describe('when a branded module is requested for a brand it does not support', function() {
		...
		before(async function () {
			const now = (new Date()).toISOString();
			response = await request(this.app)
				.get(`/__origami/service/build/v2/bundles/css?modules=${moduleName}&newerthan=${now}&brand=${brand}`)
				.redirects(5)
				.set('Connection', 'close');
		});
	...
	describe('when a module is requested with an invalid source param', function () {
		...
		before(async function () {
			const now = (new Date()).toISOString();
			response = await request(this.app)
				.get(`/__origami/service/build/v2/bundles/css?modules=${moduleName}&newerthan=${now}&source=${source}`)
				.redirects(5)
				.set('Connection', 'close');
		});
	...
	describe('when the modules parameter is missing', function() {
		...
		before(async function () {
			response = await request(this.app)
				.get('/__origami/service/build/v2/bundles/css')
				.redirects(5)
				.set('Connection', 'close');
		});
	...
	describe('when the modules parameter is not a string', function() {
		...
		before(async function () {
			response = await request(this.app)
				.get('/__origami/service/build/v2/bundles/css?modules[]=foo&modules[]=bar')
				.redirects(5)
				.set('Connection', 'close');
		});
	...
	describe('when an origami component which does not follow v1 of of the specification is requested', function() {
		...
		before(async function () {
			response = await request(this.app)
				.get(`/__origami/service/build/v2/bundles/css?modules=${moduleName}`)
				.redirects(5)
				.set('Connection', 'close');
		});
