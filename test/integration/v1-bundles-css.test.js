...
describe('GET /v1/bundles/css', function() {
	...
	describe('when a module is requested', function() {
		...
		it('should respond with a v2 `Location` header', function() {
			assert.deepEqual(response.headers['location'], `/__origami/service/build/v2/bundles/css?modules=${moduleName}&newerthan=${now}`);
		});
...
describe('GET /__origami/service/build/v1/bundles/css', function() {
	this.timeout(20000);
	this.slow(5000);
	...
	describe('when a module is requested', function() {
		...
		before(async function () {
			response = await request(this.app)
				.get(`/__origami/service/build/v1/bundles/css?modules=${moduleName}&newerthan=${now}`)
				.set('Connection', 'close');
		});
	...
	if (!process.env.HOST) {
		describe('when a module is requested that has a static bundle', function() {
			...
			before(async function () {
				response = await request(this.app)
					.get(`/__origami/service/build/v1/bundles/css?modules=${moduleName}`)
					.set('Connection', 'close');
			});
			...
			it('should ignore URL encoding when checking for static bundles', function() {
				const pathUnencoded = `/__origami/service/build/v1/bundles/css?modules=${moduleName}^1.0.0`;
				const pathEncoded = `/__origami/service/build/v1/bundles/css?modules=${moduleName}%5E1.0.0`;
				const expectedContent = '/* STATIC BUNDLE (v1/bundles/css) */\n';
