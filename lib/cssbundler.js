...
CssBundler.prototype = {
	...
	_compileSassFiles: Q.async(function* (installation, requestedComponents, allComponents) {
		const variables = {};
		...
		for (const name in allComponents) {
			if (allComponents.hasOwnProperty(name)) {
				...
				// If the requested module is branded, only allow if it explicitly supports the requested brand.
				let supportsBrand = true;
				let brands = [];
				if (isRequested) {
					const manifest = yield installation.getOrigamiManifest(name);
					supportsBrand = manifest && !manifest.brands || manifest.brands.includes(brand);
					brands = manifest && manifest.brands ? manifest.brands : [];
				}

				if (!oLayoutException && !supportsBrand) {
