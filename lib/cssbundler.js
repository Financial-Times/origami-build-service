...
CssBundler.prototype = {
	...
	createMainFile: Q.async(function* (installation, moduleset, mainScriptPath, options) {
		...
		for (const name in mainSassDescription.variables) {
			if (Object.prototype.hasOwnProperty.call(mainSassDescription.variables, name)) {
	...
	_doBuild: Q.async(function*(installation, mainFilePromise, outputPath, outputFileName, options) {
		...
		return new Promise(function(resolve, reject) {
			...
			const buildStream = obt.build.sass(gulp, log, obtOptions);
	...
	_readCssFiles: function(installation, requestedComponents) {
		...
		for (const name in requestedComponents) {
			if (Object.prototype.hasOwnProperty.call(requestedComponents, name)) {
				...
				for (const i in paths) {
					if (Object.prototype.hasOwnProperty.call(paths, i)) {
						readpromises.push(fs.readFile(paths[i]));
					}
				}
			}
		}
	...
	_compileSassFiles: Q.async(function* (installation, requestedComponents, allComponents, options) {
		...
		for (const name in allComponents) {
			if (Object.prototype.hasOwnProperty.call(allComponents, name)) {
				...
				if (Object.prototype.hasOwnProperty.call(allComponents, 'o-layout')) {
		...
		if (oAssetsPresent) {
			...
			for (const assetPath in assetsPaths) {
				if (Object.prototype.hasOwnProperty.call(assetsPaths, assetPath)) {
