...
const CssObtBundler = require('./cssobtbundler');
...
Bundler.prototype = {
	...
	getBundle: Q.async(function*(type, installationPromise, moduleSet, options) {
		...
		// HACK: For CSS, the v2 endpoint is different - so we need to cache
		// separately here.
	...
	_createContentStream: function(type, installationPromise, moduleSet, options) {
		...
		} else if (type === 'css' && !options.useobt) {
			bundler = new CssBundler();
		} else if (type === 'css' && options.useobt === true) {
			bundler = new CssObtBundler();
		} else if (type === 'demo') {
