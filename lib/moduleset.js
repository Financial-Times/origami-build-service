...
ModuleSet.parseModuleName = function(fullModuleName) {
	...
	const moduleParseErrorMessage = 'Unable to parse module name: ' + fullModuleName;
	if (!moduleNameParts) {
		throw new UserError(moduleParseErrorMessage);
	}
	let moduleName = moduleNameParts[1];
	const version = moduleNameParts[2] || '*';
	const mainPathOverride = moduleNameParts[3];
	let endpoint;
	try {
		endpoint = EndpointParser.decompose(moduleName + '#' + version);
	} catch (error) {
		throw new UserError(moduleParseErrorMessage);
	}
...
function moduleNameToBowerCompatibleName(moduleName) {
	...
	if (!name) {
		throw new UserError('Unable to parse module name: ' + moduleName);
	}
