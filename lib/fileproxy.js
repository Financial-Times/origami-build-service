...
FileProxy.prototype = {
	getContent: Q.async(function*(fileInfo, doVersionLock, reqUrl) {
		if (doVersionLock) {
			...
			return new Buffer(this.versionLockBuildserviceUrls(yield pfs.readFile(fileInfo.path, 'utf8'), componentName, installed[componentName].version, 'https://' + hostnames.preferred + reqUrl));
		} else {
	...
	getFileInfo: Q.async(function*(url) {
		...
		if (this.registry) {
			...
			try {
				...
				const tmp = new HTTPError(503, 'Unable to read packages from \'' + this.registry.registryURL + '\': ' + e.message);
				tmp.parentError = e;
				throw tmp;
			}
			...
			if (!packageListByURL[manifest._source]) {
				throw new HTTPError(403, 'Refusing to serve files from \'' + componentName + '\', because \'' + manifest._source +
					'\' is not in the (cached) list of known packages at \'' + this.registry.registryURL + '\'.\n');
			}
		}
