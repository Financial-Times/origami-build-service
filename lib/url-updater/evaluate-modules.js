...
module.exports = async function evaluateModules(modules) {
	...
	for (const module of modules) {
		...
		const lastMajor = semver.coerce(semver.major(latestVersion));
		...
		const belowLatestMajorRelease = semver.gtr(lastMajor, requestedVersion, { coerce: true });
		const npmOnlyUpgrade = semver.eq(lastMajor, semver.inc(lastBowerMajor, 'major'));
