...
module.exports = app => {
	...
	app.post([
		'/__origami/service/build/url-updater/',
	 ], addNavigationToRequest(), express.urlencoded({ extended: false }), express.text(), async (request, response) => {
		try {
			...
			const v2AutoInit = url.searchParams.has('autoinit') === false || url.searchParams.get('autoinit') !== '0';
