	...
	app.post('/url-updater/', cacheControl({ maxAge: 0 }), addNavigationToRequest(), express.urlencoded({ extended: false }), express.text(), async (request, response) => {
		try {
			...
			const npmUpgrades = results
				.some(component => component.npmNextUpgrade);
			const postNpmUpgrades = results
				.some(component => component.postNpmUpgrade);

			let updatedBuildServiceUrl = await updateUrlForResults(url, results);
			updatedBuildServiceUrl = decodeURIComponent(updatedBuildServiceUrl.toString());
