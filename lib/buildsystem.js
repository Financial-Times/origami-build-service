...
function getBundleDetails(req) {
	...
	return {
		...
		// Hack: to avoid introducing extra options, useobt is added as a
...
function BuildSystem(options) {
	...
	this.installationManager = new InstallationManager({
		...
		whitelist: options.whitelist || { 'github.com': true }
	});
	...
	outputBundle: function(type, req, res, next) {
		...
		return Promise.race([
			...
			if (req.query.redirects) {
				...
				if (req.path.match(/^\/(v1|bundles)\//)) {
					res.set('X-Service-Termination-Date', 'Mon, 1 Mar 2016 12:00:00 GMT');
				}
