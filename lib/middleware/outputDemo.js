...
const extractMinimalHtml = (fullHtml) => {
	...
	let formattedHTML;
	try {
		formattedHTML = prettier.format(body.html().trim(), { parser: 'html' });
	} catch {
		// HTML may fail to format if it is invalid. Maintain current behaviour
		// and return invalid demo HTML. Future versions of the component
		// demos and the Build Service may error instead.
	}
	return formattedHTML || body.html().trim();
};
