...
const surrogateControlHeaderFromExpiry = require('../utils/surrogateControlHeaderFromExpiry');
...
const extractMinimalHtml = (fullHtml) => {
	...
	return prettier.format(body.html().trim(), { parser: 'html' });
};
			...
			.then(function(bundle) {
				...
				if (req.query.redirects) {
					...
					res.set({
						...
						'Surrogate-Control': surrogateControlHeaderFromExpiry(
							bundle.expiryTime
						),
					});
