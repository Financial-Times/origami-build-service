...
const extractMinimalHtml = (fullHtml) => {
	...
	// Grab the first demo chunk
	const demoChunk = $('.o-registry-demo-chunk').first();
	if (demoChunk.length) {

		// Remove the demo class and class attribute if required
		demoChunk.removeClass('o-registry-demo-chunk');
		if (demoChunk.attr('class') === '') {
			demoChunk.removeAttr('class');
		}

		// Remove all siblings of the demo chunk so that we can
		// grab the parent inner HTML
		for (const sibling of demoChunk.siblings().toArray()) {
			$(sibling).remove();
		}

		// Return the chunk HTML
		return demoChunk.parent().html().trim();
	}

	// If no demo chunk was found, use the body
	const body = $('body');
