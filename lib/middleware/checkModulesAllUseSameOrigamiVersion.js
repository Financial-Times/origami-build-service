...
function getOrigamiVersion(name, version) {
	if (origamiVersion1Modules[name]) {
		...
		// If the requested version is not a subset of the versions which are built against Origami Spec v1
		// and the requested version intersects with the versions which are built against Origami Spec v1
		// then we throw an error because we do not know if the bundle will end up with components which
		// are built according to different versions of the Origami Spec, which is something we have chosen
		// to not support in the Origami Build Service.
