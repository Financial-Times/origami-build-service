...
function getOrigamiVersion(name, version) {
	if (origamiVersion1Modules[name]) {
		...
		// TODO: What should we do if the requested version intersects with versions
		// which are spec v1 but is not a full subset?
		// E.G. If component `o-no` versions upto and including v4 are built against
		// origami spec v1 and from v5 onwards is built againt origami spec v2 it's
		// version range which covers spec v1 is `<= 4`. If the version range being
		// requested is `>= 4`, what should we do? This range can return a spec v1
		// or a spec v2 version and the result is dependent upon whether any other
		// modules in the dependency tree require of `o-no`.
		if (semver.subset(version, origamiVersion1Modules[name])) {
			return 1;
		}

		if (semver.intersects(version, origamiVersion1Modules[name])) {
			throw new Error(`The version range ${version} for ${name} is not allowed because it is ambiguous, it can return versions of the component which were built against different versions of the Origami Specification.`);
		}
	}
...
module.exports = function checkModulesAllUseSameOrigamiVersion() {
	...
	return (request, response, next) => {
		...
		if (modules) {
			...
			const moduleWithOrigamiVersion = [];
			for (const [name, version] of parsedModules) {
				try {
					const origamiVersion = getOrigamiVersion(name, version);
					moduleWithOrigamiVersion.push([origamiVersion, name, version]);
				} catch (error) {
