		...
		if (request.query.modules) {
			...
			for (const [name, version] of parseModulesParameter(request.query.modules)) {
				if (!allowedModules.includes(name)) {
					...
					return next(
						httpError(400, `An unrecognised component, "${name}", was included in the module parameter. An allow list of components has been added for security reasons. Please check your typos or speak to the Origami team for help.`)
					);
				}
			}
