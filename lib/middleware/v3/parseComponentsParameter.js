...
const parseComponentsParameter = components => {
	...
	for (const name of componentNames) {
		if (!name.startsWith('@financial-times/')) {
			throw new UserError(
				`The components query parameter can only contain components from the @financial-times namespace. Please remove the following from the components parameter: ${name}.`
			);
		}
	}
