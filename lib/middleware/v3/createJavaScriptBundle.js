...
const { bundleJavascriptEsbuildPlugin } = require('./bundleJavascriptEsbuildPlugin');
...
const createJavaScriptBundle = async (request, response) => {
	...
	try {
		...
		let bundle;
		if (request.query.esbuildplugin) {
			response.startTime('bundleJavascriptEsbuildPlugin');
			bundle = await bundleJavascriptEsbuildPlugin(bundleLocation);
			response.endTime('bundleJavascriptEsbuildPlugin');
		} else {
			response.startTime('bundleJavascript');
			bundle = await bundleJavascript(bundleLocation);
