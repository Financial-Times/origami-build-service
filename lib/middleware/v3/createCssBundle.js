 ...
 * @param {string} brand The brand to compile the css for. E.g. "core", "internal", or "whitelabel".
...
async function retrieveCssBundle(components, brand, systemCode, npmRegistryURL) {
	...
	for (const [name, version] of Object.entries(components)) {
		...
		if (await isOrigamiV2Component(location) === false) {
			throw new UserError(`${name.replace('@financial-times/', '')}@${version} is not an Origami v2 component, the Origami Build Service v3 API only supports Origami v2 components.`);
		}
	}
