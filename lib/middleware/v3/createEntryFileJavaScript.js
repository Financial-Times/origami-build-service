...
async function createEntryFileJavaScript(installationDirectory, components, callback) {
	...
	for (const name of componentNames) {
		...
		const addComponentToOrigamiGlobal =
			...
			`self.Origami["${nonNamespaceName}"].default = ${camelCasedName}.default; // With the .default property for backward compatibility.`;

		if (callback) {
			const addComponentToComponentsVariable =
				`components["${nonNamespaceName}"] = ${camelCasedName}.default || ${camelCasedName};\n` +
				`components["${nonNamespaceName}"].default = ${camelCasedName}.default; // With the .default property for backward compatibility.`;
