...
const parseModulesParameter = modules => {
	...
	for (const [module, version] of m.entries()) {
		if (semver.validRange(version)) {
			...
			errors.push(`The version ${module} in ${module}@${version} is not a valid version.`);
		}
	}
...
module.exports = {
    ...
    parseModulesParameterExpressMiddleware: function(request, response, next) {
        const modules = parseModulesParameter(request.query.modules);
        response.locals.modules = modules;
        next();
    }
};
