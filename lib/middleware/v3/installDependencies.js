...
async function installDependencies(location, registry = 'https://registry.npmjs.org/') {
	try {
		try {
			...
			await execa.command(
				`${npm} install --production --ignore-scripts --no-package-lock --no-audit --prefer-offline --progress=false --fund=false --package-lock=false --strict-peer-deps --update-notifier=false --bin-links=false --registry=${registry} --cache=${npmCacheLocation}`,
				{
					cwd: location,
					preferLocal: false,
					shell: true,
