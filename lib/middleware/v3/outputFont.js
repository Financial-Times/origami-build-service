...
const outputFont = async (request, response) => {
	try {
		...
		if (isFontDeprecated(fontFileName.split('.')[0])) {
			...
			response.setHeader('Cache-Control', 'max-age=300, must-revalidate, no-cache');
			response.status(410);

			response.send(JSON.stringify(`Origami Build Service returned an error: The font '${fontFileName}' is deprecated, we no longer have a license to use it. Please contact Origami Team for advice on which font to use instead.`));
			return;
		}
