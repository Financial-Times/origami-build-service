...
async function retrieveFont(version, fontFileName, fontFormat, npmRegistryURL) {
	...
	if (fileExists(fontFilePath)) {
		...
		throw new UserError(
			`${componentName}@${version} does not contain a font named '${fontFileName}' with format '${fontFormat}'.`
		);
	}
}
...
const outputFont = async (request, response) => {
	try {
		...
		const fontFilePath = await memoizedRetrieveFont(version, fontFileName, fontFormat, request.app.ft.options.npmRegistryURL);
