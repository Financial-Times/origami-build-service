...
module.exports = {
	...
	buildDemo: async function buildDemo(location, brand, demoName, componentName, version) {
		...
		if (demoBuildConfig.sass) {
			css = await buildDemoSass(buildConfig, demoName, componentName, version);
		}
		...
		return await buildDemoHtml(buildConfig, css, js, demoName, componentName, version);
	}
};
...
async function buildDemoSass(buildConfig, demoName, componentName, version) {
	...
	if (!exists) {
		throw new ComponentError(`${componentName}@${version}'s demo Sass file not found:  ${src}`);
	}
