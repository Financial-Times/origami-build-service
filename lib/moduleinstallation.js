...
const AsyncCollection = require('./utils/asynccollections');
 ...
 * Options:
 * 	- whitelist: optional. Object in format {hostname:true} that lists hosts allowed to contain non-origami modules, or a special value "*" that means no restrictions.
 *
 * @param {Object} options
 */
function ModuleInstallation(moduleset, options) {
	if (!bower) bower = require('bower');
	if (!EndpointParser) EndpointParser = require('bower-endpoint-parser');
	...
	this.nonOrigamiWhitelist = options.whitelist || { 'github.com': true };

	this.dirInit = Q.async(function*(){
		const dir = this.options.dir;
		yield pfs.makeTree(dir);
		this.options.dir = yield pfs.canonical(dir);
	}).call(this);
...
ModuleInstallation.prototype = {
		...
		try {
			...
			installPromise.progress(function(logMsg) {
				if (logMsg.level !== 'info' && logMsg.level !== 'warn' && logMsg.level !== 'action') {
					log.debug({bowerLogEvent: logMsg}, 'Bower log event');
				}
			});
			...
			log.debug({dest: this.options.dir}, 'Completed module install');
			return installed;
		} catch(err) {
	...
	_addModulesToBowerManifest: Q.async(function*(modules) {
		...
		let bowerManifest;
		if (yield pfs.exists(manifestPath)) {
			bowerManifest = JSON.parse(yield pfs.read(manifestPath));
		...
		return pfs.write(manifestPath, JSON.stringify(bowerManifest));
	}),
	...
	_ensureOnlyWhitelistedModulesInstalled: Q.async(function*(installed){
		if (this.nonOrigamiWhitelist === '*') return;

		for(let componentName in installed) {
			if (installed.hasOwnProperty(componentName)) {
				const url = installed[componentName].pkgMeta._source;
				const hostname = this._getHostname(url);

				if (!hostname || !this.nonOrigamiWhitelist[hostname]) {
					...
					if (!manifestPath) {
						const msg = componentName + ' is not an Origami module and ' + (hostname || 'local path') + ' is not allowed to host non-Origami modules.\n' +
							...
							'If you don\'t want to add origami.json, then you must host ' + componentName + ' on ' + Object.keys(this.nonOrigamiWhitelist).join(' or ') + '.';
						log.error(msg);
						this.destroy();
	...
	listAllOrigamiComponents: function() {
		const hasOrigamiManifest = this.hasOrigamiManifest.bind(this);
		return this.listAll().then(function(allComponents) {
			return AsyncCollection.filter(Object.keys(allComponents), hasOrigamiManifest).then(function(origamiComponents) {
				// The filter operates on the keys Object.keys.  This converts back to the original objects
				const componentMap = {};
				origamiComponents.forEach(function(component) {
					componentMap[component] = allComponents[component];
				});
				return componentMap;
			});
		});
	...
	listAll: function() {
		if (!this._listCache) {
			this._listCache = this._listComponents(false);
		}
	...
	list: function() {
		return this.listAll().then(function(allComponents){
			const components = {};
			this.moduleSet.getResolvedModules().forEach(function(m){
				const name = m.endpoint.name;
	...
	_listComponents: Q.async(function*(){
		...
		const listed = {};
		for(let componentName in mainPathGlobs) {
			if (mainPathGlobs.hasOwnProperty(componentName)) {
				...
				for(let j in paths) {
					...
					if ((yield pfs.stat(paths[j])).isDirectory()) {
						yield Q.all(['js','css','scss'].map(function(extension){
							...
							return pfs.exists(mainPath).then(function(exists){
	...
	getBowerManifest: function(installedBowerComponentName) {
		...
		return pfs.read(componentPath + '/.bower.json')
			.then(function(json){
	...
	_origamiManifestPathForDirectory: Q.async(function*(dir) {
		...
		if (yield pfs.exists(newPath)) return newPath;

		const oldPath = dir + '/.origamiconfig'; // backward compat only
		if (yield pfs.exists(oldPath)) return oldPath;
	...
	getOrigamiManifest: Q.async(function*(componentName) {
		try {
			...
			return JSON.parse(yield pfs.read(yield this.getOrigamiManifestPath(componentName)));
		}
		catch(e) {
