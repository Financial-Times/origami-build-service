...
function ModuleInstallation(moduleset, options) {
	...
	this.dirInit = Q.async(function*(){
		...
		this.options.dir = yield fs.realpath(dir);
	}).call(this);
...
ModuleInstallation.prototype = {
	...
	_addModulesToBowerManifest: Q.async(function*(modules) {
		...
		if (yield fs.pathExists(manifestPath)) {
			bowerManifest = JSON.parse(yield fs.readFile(manifestPath, 'utf8'));
		...
		return fs.outputFile(manifestPath, JSON.stringify(bowerManifest));
	}),
	...
	_ensureOnlyWhitelistedModulesInstalled: Q.async(function*(installed){
		...
		for(const componentName in installed) {
			if (Object.prototype.hasOwnProperty.call(installed, componentName)) {
	...
	_listComponents: Q.async(function*(){
		...
		for(const componentName in mainPathGlobs) {
			if (Object.prototype.hasOwnProperty.call(mainPathGlobs, componentName)) {
				...
				for(const j in paths) {
					...
					if ((yield fs.stat(paths[j])).isDirectory()) {
						yield Q.all(['js','css','scss'].map(function(extension){
							const mainPath = paths[j] + '/main.' + extension;
							return fs.pathExists(mainPath).then(function(exists){
	...
	_globsToPaths: Q.async(function*(globs, componentName) {
		...
		for(const i in globs) {
			if (Object.prototype.hasOwnProperty.call(globs, i)) {
				...
				for(const j in matched) {
					if (Object.prototype.hasOwnProperty.call(matched, j)) {
	...
	getBowerManifest: function(installedBowerComponentName) {
		...
		return fs.readFile(componentPath + '/.bower.json', 'utf8')
			.then(function(json){
	...
	_origamiManifestPathForDirectory: Q.async(function*(dir) {
		...
		if (yield fs.pathExists(newPath)) return newPath;

		const oldPath = dir + '/.origamiconfig'; // backward compat only
		if (yield fs.pathExists(oldPath)) return oldPath;
	...
	getOrigamiManifest: Q.async(function*(componentName) {
		try {
			...
			return JSON.parse(yield fs.readFile(yield this.getOrigamiManifestPath(componentName), 'utf8'));
		}
		catch(e) {
	...
	getConfig: function() {
		...
		return {
			...
			registry: {
				search: [
					this.registry.bowerRegistryURL,
