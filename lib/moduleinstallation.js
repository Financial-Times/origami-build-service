...
ModuleInstallation.prototype = {
	...
	install: Q.async(function*() {
		...
		try {
			...
			// TODO - Pass through current brand.
			const brand = 'not-supported';
			yield this._ensureOnlyWhitelistedModulesInstalled(installed);
			yield this._ensureOnlyBrandSupportedModules(modules, installed, brand);
	...
	/**
	 * Checks the modules installed support the current brand.
	 */
	_ensureOnlyBrandSupportedModules: Q.async(function* (modules, installed, brand){
		const log = this.log;
		for (const index in modules) {
			if (modules.hasOwnProperty(index)) {
				const module = modules[index];
				const componentName = module.moduleName;
				const manifest = yield this.getOrigamiManifest(componentName);
				// Check o-layout < 3.0.0 is not installed.
				// If it is allow the build regardless of the set brand.
				...
				if (installed['o-layout']) {
					const oLayoutVersion = installed['o-layout'].pkgMeta ? installed['o-layout'].pkgMeta.version : null;
					oLayoutException = parseFloat(oLayoutVersion) < 3;
				}
				// Allow modules if they are unbranded.
				// If the module is branded, only allow if it explicitly supports the requested brand.
				if (!oLayoutException && manifest && manifest.brands && !manifest.brands.includes(brand)) {
					const msg = componentName + ' does not support the ' + brand + ' brand.';
					log.error(msg);
					this.destroy();
