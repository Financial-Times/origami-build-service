...
function buildHtml(gulp, log, buildConfig) {
	...
	log.info('Rendering: ' + dest + destName);
...
module.exports = function (gulp, log, config) {
	...
	if (configPath) {
		log.info('Building build service' + ' demos (config: ' + configPath + ')');
		...
		if (!configPath.includes('origami.json')) {
			log.info('Please move your demo config into origami.json following the spec: http://origami.ft.com/docs/syntax/origamijson');
		}
		...
		demos.map(function(demo) {
			log.info('Building demo ' + demo.name);
			...
			demoStream
				.add(buildHtml(gulp, log, buildConfig)
